...
const getName = async (name) => {
  ...
  let arrayNames = apiDbGames.filter((g) =>
...
const getPlatforms = async () => {
  ...
  const plat = [];
  box.forEach((e) =>
    e.platforms.forEach((a) => {
      if (!a.createdAtDb && !plat.includes(a.platform.name)) {
        plat.push(a.platform.name);
      }
    })
  );
  return plat;
...
const getId = async (id) => {
  try {
    ...
    throw new Error("No se encontro un videoJuego con el id propocionado");
  }
};
...
router.get("/:idVideogame", async (req, res) => {
  ...
  try {
    let db = await dbGame(idVideogame);
    ...
    try {
      ...
      return res
        .status(400)
        .send(
          `la api de videoGames respondio con ${error.message} y la base de datos respondio con ${error2.message}`
        );
    }
  }
});
...
router.get("/platforms/:platform", async (req, res) => {
  ...
  let b = a.filter((e) => {
    if (
      !e.createdAtDb &&
      e.platforms.filter((a) => {
        if (a.platform.name === platform) return a;
      }).length
    )
      return e;
    if (e.createdAtDb && e.platforms.includes(platform)) {
      return e;
    }
  });
  console.log(b);
